...
public class JwtAuthenticationFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (PUBLIC_ENDPOINTS.stream().anyMatch(path::startsWith)) {
            System.out.println("🟡 [JWT Filter] Public endpoint – skipping auth: " + path);
        ...
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            System.out.println("🔴 [JWT Filter] Missing or invalid token.");
        ...
        try {
            ...
            // ✅ Mutate request → add custom headers for downstream services
        ...
        return chain.filter(exchange)
                .doOnSuccess(aVoid -> System.out.println("✅ [JWT Filter] Request forwarded successfully → " + path))
